...
capture_output_success ls_remote       git -C ${TMP_WORKDIR}/gitcache ls-remote
capture_output_success ls_remote_stats -s

capture_output_success ls_remote_with_url       git -C ${TMP_WORKDIR}/gitcache ls-remote https://github.com/seeraven/gitcache.git
capture_output_success ls_remote_with_urlstats -s

capture_output_success ls_remote_with_ref       git -C ${TMP_WORKDIR}/gitcache ls-remote origin
capture_output_success ls_remote_with_refstats -s

export GITCACHE_UPDATE_INTERVAL=3600
capture_output_success ls_remote_without_update       git -C ${TMP_WORKDIR}/gitcache ls-remote
...
capture_output_success ls_remote_non_mirror git -C ${TMP_WORKDIR}/gitcache5 ls-remote
