...
class GitCacheCallCommandRetryTest(TestCase):
    ...
    def test_timeout(self):
        ...
        if self.on_windows:
            cmd = ['cmd.exe', '/C', 'timeout /t 2 /nobreak >nul']
        else:
            cmd = ['sleep', '2s']
        return_code, stdout_buffer, stderr_buffer = call_command_retry(cmd, 3,
                                                                       command_timeout=1)
    ...
    def test_shell_output_timeout(self):
        ...
        if self.on_windows:
            cmd = 'echo a & timeout /t 1 /nobreak >nul & echo b & '
            cmd += 'timeout /t 3 /nobreak >nul & echo c'
        else:
    ...
    def test_abort_pattern(self):
        ...
        return_code, stdout_buffer, _ = call_command_retry(
            cmd, 3, abort_on_pattern=b'abort_marker')
        self.assertEqual(-3000, return_code)
        self.assertIn(b'abort_marker', stdout_buffer)

        return_code, stdout_buffer, _ = call_command_retry(
            cmd, 3, abort_on_pattern=b'abort_markerddddd')
        self.assertEqual(1, return_code)
        self.assertIn(b'abort_marker', stdout_buffer)
    ...
    def test_shell_abort_pattern(self):
        ...
        if self.on_windows:
            ...
            cmd = 'echo a; echo abort_marker; exit 1'
        return_code, stdout_buffer, _ = call_command_retry(
            cmd, 3, shell=True, abort_on_pattern=b'abort_marker')
        self.assertEqual(-3000, return_code)
        self.assertIn(b'abort_marker', stdout_buffer)

        return_code, stdout_buffer, _ = call_command_retry(
            cmd, 3, shell=True, abort_on_pattern=b'abort_markerddddd')
        self.assertEqual(1, return_code)
        self.assertIn(b'abort_marker', stdout_buffer)
