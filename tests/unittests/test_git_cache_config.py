...
class GitCacheConfigTest(TestCase):
    ...
    def test_defaults(self):
        ...
        self.assertEqual(config.get("MirrorHandling", "CleanupAfter"),
                         14 * 24 * 60 * 60)
        self.assertEqual(config.get("LFS", "Retries"),
                         3)
        self.assertEqual(config.get("LFS", "CommandTimeout"),
                         60 * 60)
        self.assertEqual(config.get("LFS", "OutputTimeout"),
                         5 * 60)
        self.assertEqual(config.get("LFS", "PerMirrorStorage"),
                         True)

    @mockenv(GITCACHE_DIR="/tmp",
             GITCACHE_CLONE_COMMAND_TIMEOUT="a")
    ...
    @mockenv(GITCACHE_DIR="/tmp",
             ...
             GITCACHE_LFS_PER_MIRROR_STORAGE="0")
